package org.freeland.wildscan.data.sync;
...
import org.freeland.wildscan.ReportWizardActivity;
...
public class WildscanSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public static final String KEY_EVENTS_ONLY = "org.freeland.wildscan.data.sync.EventsOnly";
    public static final String KEY_FIRST_TIME = "org.freeland.wildscan.data.sync.FirstTime";
    public static final String KEY_SYNC_MARKER = "org.freeland.wildscan.data.sync.SyncMarker";
    public static final String SYNC_PROGRESS_ACTION = "org.freeland.wildscan.data.sync.SyncProgress";
    public static final String KEY_SYNC_PROGRESS_MESSAGE = "org.freeland.wildscan.data.sync.SyncStatus";
...
//        } else {
*/
        ok = syncTables(lastSync, syncResult);
...
//		synchronized(mDataManager) { mDataManager.isSyncing = false; }
    ...
    private boolean syncTables(long lastSync, SyncResult syncResult) {
        int status = SYNC_SYNCING_STATIC;
        sendProgressBroadcast(status++);
        boolean ret = syncStaticContent(lastSync, syncResult);
        for (int tbl = 0; tbl < SYNC_N_TBLS; tbl++) {
            sendProgressBroadcast(status++);
            ret = ret && syncTable(lastSync, tbl, syncResult);
        }
        return ret;
    }

    private boolean syncStaticContent(long lastSync, SyncResult syncResult) {
        Uri tableUri = null;
        ...
        try {
            ...
            reader = new JsonArrayReader(mContext, StaticContent.TABLE_NAME, lastSync);
            while (reader.hasNext()) {
    ...
    private boolean syncTable(long lastSync, int tblIdx, SyncResult syncResult) {
        ArrayList<ContentProviderOperation> ops = new ArrayList<ContentProviderOperation>();
        String toDelete;
        Uri tableUri = null;
        boolean changed = false, ok = true;
        ...
        try {
            ...
            reader = new JsonArrayReader(mContext, SYNC_TBL_NAMES[tblIdx], lastSync);
    ...
    private boolean syncImages(String table, String field, SyncResult syncResult, boolean isFirstTime) {
        ...
        if (c.moveToFirst()) {
            ...
            do {
                ...
                if (++i % 10 == 0)
                    sendProgressBroadcast(SYNC_SYNCING_PHOTOS, 0.5 * i / count);
