package org.freeland.wildscanlaos.data.provider;
...
public class WildscanDataProvider extends ContentProvider {
    ...
    public static final String AUTHORITY = "org.freeland.wildscanlaos.data.provider";
...
//            c = db.query(mTable, projection, mSelection, selectionArgs, null, null, sortOrder);
    ...
    private synchronized Cursor queryWithTranslation(String table, String[] columns,
        ...
        if (table==Species.TABLE_NAME) {
            rawQuery += " ORDER BY common_name COLLATE LOCALIZED ASC";
        }
        if (rawQuery != null) {
...
//			if (i<columns.length-1) rawQuery += ",";
        ...
        rawQuery += " FROM " + mTable + " o LEFT OUTER JOIN (SELECT * FROM " + transTable + " WHERE " + langField + "=" + DatabaseUtils.sqlEscapeString(lang) + ") t " +
                "ON o." +
                idField + "=t." + refIdField;
